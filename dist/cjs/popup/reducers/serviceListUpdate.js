...
function serviceListUpdate(state, action) {
    if (action.type === updateServiceList_1.UPDATE_SERVICE_LIST) {
        return tslib_1.__assign(tslib_1.__assign({}, state), { accounts: action.accounts, view: tslib_1.__assign(tslib_1.__assign({}, state.view), { isLoading: false, hasError: false }) });
    }
    else {
        return state;
    }
